    ...
    Public Class frmPayrollRecord_HB
        ...
        Private Sub txtEmployeeName_TextChanged(sender As Object, e As System.EventArgs)
            Try
                RestaurantPOS14.ModClasses.con = New System.Data.SqlClient.SqlConnection(RestaurantPOS14.ConnectionString.cs)
                RestaurantPOS14.ModClasses.con.Open()
                RestaurantPOS14.ModClasses.cmd = New System.Data.SqlClient.SqlCommand("select Payroll.ID as [ID],RTRIM(PaymentID) as [Payment ID],RTRIM(Registration.UserID) as [Employee ID],RTRIM(Name) as [Employee Name],RTRIM(SSN) as [SIN],Hours as [Working Hours],Rate as [Rate/Hour],GrossSalary as [Gross Pay],CPPPer as [CPP %],CPP as [CPP],EIPer as [EI %],EI as [EI],FedTaxPer as [FedTax %],FedTax as [Fed Tax],VPPer as [VP %],VP as [VP],Convert(DateTime,paymentdate,131) as [Payment Date],RTRIM(PaymentMode) as [Payment Mode],RTRIM(Remarks) as [Payment Mode Details],NetPay as [Net Pay] from Payroll,Registration where Registration.UserID=Payroll.UserID and Name like N'%" & Me.txtEmployeeName.Text & "%' order by paymentdate", RestaurantPOS14.ModClasses.con)
                Dim sqlDataAdapter As System.Data.SqlClient.SqlDataAdapter = New System.Data.SqlClient.SqlDataAdapter(RestaurantPOS14.ModClasses.cmd)
                Dim dataSet As System.Data.DataSet = New System.Data.DataSet()
                sqlDataAdapter.Fill(dataSet, "Payroll")
                sqlDataAdapter.Fill(dataSet, "Registration")
